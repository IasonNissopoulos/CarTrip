...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        cartrip_models = inspect.getmembers(models, inspect.isclass)
        ModelToSeed = None
        for cartrip_model in cartrip_models:
            if cartrip_model[0] == options['model']:
                ModelToSeed = cartrip_model[1]
                break
        if not ModelToSeed:
            return
